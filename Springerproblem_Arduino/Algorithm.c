...
ResultPath* resultPath_initialize(Result* result)
{
    ...
    unsigned char boardSize = board_getSize(result->board);
    path->length = 3 * (boardSize * boardSize);
    path->data = malloc(path->length);
    memset(path->data, '\x20', path->length);

    for(unsigned char pathIndex = 0; pathIndex < boardSize*boardSize; ++pathIndex)
    {
        for(unsigned char boardIndex = 0; boardIndex < boardSize*boardSize; ++boardIndex)
        {
...
Result* tryPath(Board* board, unsigned char x, unsigned char y, unsigned char n, bool closedTour)
{
    ++iterationCount;

    unsigned char boardSize = board_getSize(board);
    board_setValue(board, x, y, n);

    if(iterationCount == (boardSize*boardSize + 1))
        out("Warnsdorf failed, resorting to backtracking\n");
    ...
    for(unsigned char i = 0; i < heuristicMoveList_getCount(list); ++i)
    {
