...
Move* move_initialize(unsigned char x, unsigned char y)
{
...
HeuristicMove* heuristicMove_initialize(Move* move, unsigned char neighbourCount)
{
...
char heuristicComparator(const void* elem1, const void* elem2)
{
...
void heuristicMoveList_sort(HeuristicMoveList* list)
{
    ...
    for(unsigned char outer = list->dataCount; outer; --outer)
    {
        bool didSwap = false;
        for(unsigned char inner = 0; inner < (outer - 1); ++inner)
        {
...
HeuristicMoveList* generateHeuristic(Board* board, MoveList* moveList, bool shouldOfferStart)
{
    ...
    for(unsigned char i = 0; i < moveList_getCount(moveList); ++i)
    {
